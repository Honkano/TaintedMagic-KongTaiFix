...
public class ItemGateKey extends Item implements IWarpingGear {
    ...
    public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean b) {
        if (stack.stackTagCompound != null) {
            ...
            list.add(
                WorldProvider.getProviderForDimension(dim)
                    .getDimensionName());
    ...
    public boolean onItemUse(final ItemStack stack, final EntityPlayer player, final World world, final int x,
        final int y, final int z, final int face, final float hitX, final float hitY, final float hitZ) {
        if (stack.stackTagCompound == null) {
            if (!world.isRemote) {
    ...
    public void sparkle(final World world, final double x, final double y, final double z) {
        final FXSparkle fx = new FXSparkle(
            world,
            x + 0.33F * world.rand.nextGaussian(),
            y + 0.5D + world.rand.nextFloat(),
            z + 0.33F * world.rand.nextGaussian(),
            1.75F,
            6,
        ...
        if (stack.stackTagCompound != null) {
            ...
            if (y > -1 && dim == world.provider.dimensionId
                ...
                HUDHandler.displayString(
                    EnumChatFormatting.RED + StatCollector.translateToLocal("key.invaliddim"),
                    300,
                    false);
            } else {
                HUDHandler
                    .displayString(EnumChatFormatting.RED + StatCollector.translateToLocal("key.error"), 300, false);
            }
        }
        return stack;
    }
