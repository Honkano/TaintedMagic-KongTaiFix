...
public class ItemThaumicDisassembler extends Item {
    ...
    public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean b) {
        ...
        list.add(
            StatCollector.translateToLocal("text.disassembler.charge") + ": "
                + EnumChatFormatting.DARK_GRAY
                + getEntropyCharge(stack) / 100
                + "/"
                + MAX_CHARGE / 100);
        list.add(StatCollector.translateToLocal("text.disassembler.mode") + ": " + getModeName(stack));
        list.add(
            StatCollector.translateToLocal("text.disassembler.efficiency") + ": "
    ...
    private boolean useHoe(final ItemStack stack, final EntityPlayer player, final World world, final int x,
        final int y, final int z, final int side) {
        if (!player.canPlayerEdit(x, y, z, side, stack)
            || !player.capabilities.isCreativeMode && getEntropyCharge(stack) < ENTROPY_USAGE_HOE) return false;
        ...
        if (side == 0 || !air || block != Blocks.grass && block != Blocks.dirt) return false;
    ...
    private void addEntropy(final ItemStack stack, final int amount) {
        ...
        stack.getTagCompound()
            .setInteger(TAG_CHARGE, getEntropyCharge(stack) + amount);
    }
