...
public class ItemKatana extends Item implements IWarpingGear, IRepairable, IRenderInventoryItem, IHeldItemHUD {
    ...
    private static final ResourceLocation TEXTURE_THAUMIUM = new ResourceLocation(
        "taintedmagic:textures/models/ModelKatanaThaumium.png");
    private static final ResourceLocation TEXTURE_VOIDMETAL = new ResourceLocation(
        "taintedmagic:textures/models/ModelKatanaVoidmetal.png");
    private static final ResourceLocation TEXTURE_SHADOWMETAL = new ResourceLocation(
        "taintedmagic:textures/models/ModelKatanaShadowmetal.png");
    private static final ResourceLocation TEXTURE_GOLDEN_SHADOWMETAL = new ResourceLocation(
        "taintedmagic:textures/models/ModelKatanaGoldenShadowmetal.png");
    ...
    public void addInformation(final ItemStack stack, final EntityPlayer player, final List list, final boolean b) {
        ...
        if (stack.getItemDamage() == 3) {
            list.add(EnumChatFormatting.GOLD + StatCollector.translateToLocal("enchantment.special.sapultimate"));
        }
    ...
    public boolean hitEntity(final ItemStack stack, final EntityLivingBase entity, final EntityLivingBase player) {
        if (!entity.worldObj.isRemote && (!(entity instanceof EntityPlayer) || !(player instanceof EntityPlayer)
            ...
            if (stack.getItemDamage() == 3) {
                entity.addPotionEffect(new PotionEffect(Potion.weakness.id, 200, 2)); // 虚弱 II，10秒
                entity.addPotionEffect(new PotionEffect(Potion.hunger.id, 400, 3)); // 饥饿 II，20秒
                entity.addPotionEffect(new PotionEffect(Potion.digSlowdown.id, 200, 3)); // 挖掘疲劳，10秒（顺便让敌人挖不了方块）
                entity.addPotionEffect(new PotionEffect(Potion.confusion.id, 200, 3));
        ...
        // Sound effect
        player.worldObj
            .playSoundAtEntity(player, "thaumcraft:swing", 0.5F + (float) Math.random(), 0.5F + (float) Math.random());
                        ...
                        if (ents != null && ents.size() > 0) {
                            for (final EntityLivingBase entity : ents) {
                                if (entity != player && entity.isEntityAlive() && !entity.isEntityInvulnerable()) {
                                    ...
                                    final Vector3 movement = TaintedMagicHelper
                                        .getVectorBetweenEntities(entity, player);
                        ...
                        if (world.isRemote) {
                            for (int a = 0; a < 18; a++) {
                                Thaumcraft.proxy.wispFX2(
                                    world,
                                    player.posX + world.rand.nextGaussian() * 0.25F,
                                    player.boundingBox.minY + 1.0F + world.rand.nextGaussian() * 0.5F,
                                    player.posZ + world.rand.nextGaussian() * 0.25F,
                                ...
                                Thaumcraft.proxy.sparkle(
                                    (float) player.posX + (float) world.rand.nextGaussian() * 0.25F,
                                    (float) player.boundingBox.minY + 1.0F + (float) world.rand.nextGaussian() * 0.5F,
                                    (float) player.posZ + (float) world.rand.nextGaussian() * 0.25F,
                                    5);
                            }
                        }
    ...
    public static ResourceLocation getTexture(final ItemStack stack) {
        switch (stack.getItemDamage()) {
            ...
            case 3:
                return TEXTURE_GOLDEN_SHADOWMETAL; // 新增
        }
        return null;
    }
    ...
    public void renderHUD(final ScaledResolution res, final EntityPlayer player, final ItemStack stack,
        ...
        final float overlay = Math.min(
            hasCooldown(stack) ? (float) getCooldown(stack) / (float) COOLDOWN_TICKS
