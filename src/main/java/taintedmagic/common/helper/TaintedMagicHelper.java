...
public class TaintedMagicHelper {
    ...
    public static AspectList getPrimals(final int amount) {
        return new AspectList().add(Aspect.FIRE, amount)
     ...
     */
    public static boolean consumeVisFromInventory(final EntityPlayer player, final AspectList aspects,
        final boolean doit) {
        final IInventory baubles = BaublesApi.getBaubles(player);

        for (int i = 0; i < 4; i++) {
            if (baubles.getStackInSlot(i) != null && baubles.getStackInSlot(i)
                .getItem() instanceof ItemAmuletVis
                && ((ItemAmuletVis) baubles.getStackInSlot(i)
                    .getItem()).consumeAllVis(baubles.getStackInSlot(i), player, aspects, doit, true))
                return true;

        }
        for (final ItemStack stack : player.inventory.mainInventory) {
            if (stack != null && stack.getItem() instanceof ItemWandCasting
                && ((ItemWandCasting) stack.getItem()).consumeAllVis(stack, player, aspects, doit, true)) return true;
        }
        return false;
    }
