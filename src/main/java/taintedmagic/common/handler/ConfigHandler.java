...
public class ConfigHandler {
    ...
    public static void initConfig() {
        ...
        WARP_WAND_REFRESH_BASE = config.getFloat(
            "WARP_WAND_REFRESH_BASE",
            "wands_and_foci",
            10000.0F,
            100.0F,
            100000.0F,
         ...
         * Focus upgrade IDs
         */
        SANITY_UPGRADE_ID = config.getInt(
            "SANITY_UPGRADE_ID",
            "wands_and_foci",
            64,
            FocusUpgradeType.types.length + 1,
            Short.MAX_VALUE,
            "The ID for the Sanity focus upgrade.");

        ANTIBODY_UPGRADE_ID = config.getInt(
            "ANTIBODY_UPGRADE_ID",
            "wands_and_foci",
            65,
            FocusUpgradeType.types.length + 1,
            Short.MAX_VALUE,
            "The ID for the Antibody focus upgrade.");

        CORROSIVE_UPGRADE_ID = config.getInt(
            "CORROSIVE_UPGRADE_ID",
            "wands_and_foci",
            66,
            FocusUpgradeType.types.length + 1,
            Short.MAX_VALUE,
            "The ID for the Corrosive focus upgrade.");

        PERSISTENT_UPGRADE_ID = config.getInt(
            "PERSISTENT_UPGRADE_ID",
            "wands_and_foci",
            67,
            FocusUpgradeType.types.length + 1,
            Short.MAX_VALUE,
            "The ID for the Persistent focus upgrade.");

        DIFFUSION_UPGRADE_ID = config.getInt(
            "DIFFUSION_UPGRADE_ID",
            "wands_and_foci",
            68,
            FocusUpgradeType.types.length + 1,
            Short.MAX_VALUE,
            "The ID for the Diffusion focus upgrade.");
