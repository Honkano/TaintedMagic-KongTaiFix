...
public class TMEventHandler {
    ...
    public void repairItems(final EntityPlayer player) {
        for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
            ...
            if (!player.worldObj.isRemote && stack != null
                ...
                && stack.isItemDamaged()) if (player.ticksExisted % 20 == 0) {
    ...
    public void applyNightVision(final EntityPlayer player) {
        ...
        if (baub.getStackInSlot(1) != null && baub.getStackInSlot(1)
            .getItem() instanceof ItemLumosRing || baub.getStackInSlot(2) != null
                && baub.getStackInSlot(2)
                    .getItem() instanceof ItemLumosRing) {
            lumos = true;
    ...
    public void modifyAttackDamage(final EntityPlayer player) {
        ...
        for (int i = 0; i < inv.getSizeInventory(); i++) {
            if (inv.getStackInSlot(i) != null && inv.getStackInSlot(i)
                ...
                final ItemWandCasting wand = (ItemWandCasting) inv.getStackInSlot(i)
                    .getItem();
    ...
    public void giveResearch(final EntityPlayer player) {
        ...
        if (!ThaumcraftApiHelper.isResearchComplete(player.getCommandSenderName(), "CREATION")
            ...
            Thaumcraft.proxy.getResearchManager()
                .completeResearch(player, "CREATION");
        ...
        if (!ThaumcraftApiHelper.isResearchComplete(player.getCommandSenderName(), "OUTERREV")
            ...
            Thaumcraft.proxy.getResearchManager()
                .completeResearch(player, "OUTERREV");
